...
void init_set(struct ordered_set *s, int id_size) {
  ...
  s->id_size = id_size;
#ifdef DEBUG
  printf("\033[22;31m\n>>>>DEBUG: Ordered_set initialized correctly with "
         "id_size %d\n\033[0m",
         s->id_size);
#endif
}
 ...
 */
int size(struct ordered_set *s) { return s->size; }
...
/* Returns whether the set is empty or not
 ...
 // }
 */
int is_empty(struct ordered_set *s) { return (s->size == 0); }

int are_equal(struct node *a, struct node *b, int id_size) {
  if (a->F - b->F == 0.) {
    int i = 0;
    while (i < id_size && (a->id)[i] == (b->id)[i]) {
      i++;
#ifdef DEBUG
  	  printf("\033[22;31m\na->id: %d, b->id: %d\033[0m",
         a->id[i], b->id[i]);
#endif
    }
    if (i == id_size)
...
#ifdef DEBUG
  printf("\033[22;31m\n>>>>DEBUG: The elements are not equal!! new: %f, prev: %f\n\033[0m",
         b->F, a->F);
#endif
  return 0;
}
...
void add(struct ordered_set *s, int *id, float F) {
  ...
  if (s->size == 0) {
    ...
    if (((s->max)->value)->F < new->F) {
      ...
      ((s->max)->ttl)--;
#ifdef DEBUG
      printf("\033[22;31m\n>>>>DEBUG: This element is identical to the "
             "maximum\n\033[0m");
#endif
      return;
    } else {
...
#ifdef DEBUG
    printf("\033[22;31m\n>>>>DEBUG: This element is greater than the "
           "median\n\033[0m");
#endif
...
struct node *get_max(struct ordered_set *s) {
  ...
  struct set_node *n = s->max;
  while (n != NULL && n->ttl >= 1) {
    n = n->next;
  }
  if (n == NULL) {
#ifdef DEBUG
    printf("\033[22;31m\n>>>>DEBUG: There is no node with TTL <= 1\n\033[0m");
#endif
...
struct node *get_and_delete_max(struct ordered_set *s) {
  ...
  if (s->size == 1) {
    ...
    if (is_even(s->size) && (medseen || n == s->med))
      s->med = s->med->prev;
    else if (is_odd(s->size) && !medseen)
      s->med = s->med->next;
    if (n == s->max) {
      ((s->max)->next)->prev = NULL;
      s->max = (s->max)->next;
#ifdef DEBUG
      printf("\033[22;31m\n>>>>DEBUG: The maximum has TTL = %d\n\033[0m", n->ttl);
#endif
    } else if (n == s->min) {
      ...
      (n->next)->prev = n->prev;
#ifdef DEBUG
      printf("\033[22;31m\n>>>>DEBUG: The element out is not very "
             "special\n\033[0m");
#endif
    }
  }
