...
void add(struct ordered_set* s, int* id, float F) {
	...
	if (s->size == 0) {
		...
		s->max = elem;
#ifdef DEBUG
		printf("\033[22;31m\n>>>>DEBUG: This element is the new maximum\n\033[0m");
#endif
		...
		s->min = elem;
#ifdef DEBUG
		printf("\033[22;31m\n>>>>DEBUG: This element is the new minimum\n\033[0m");
#endif
		...
		while(n->prev != NULL && n->prev != s->med && ((n->prev)->value)->F < new->F) {
			n = n->prev;
		}
		if (n->prev != NULL && are_equal((n->prev)->value,new,s->id_size)) {
			((n->prev)->ttl)--;
			return;
		}
		elem->prev = n->prev;
		elem->next = n;
		if (n->prev != NULL) (n->prev)->next = elem;
		n->prev = elem;
#ifdef DEBUG
		printf("\033[22;31m\n>>>>DEBUG: This element is greater than the median\n\033[0m");
#endif
		if (is_odd(s->size)) s->med = s->med->next;
	} else if (are_equal((s->med)->value,new,s->id_size)) {
		((s->med)->ttl)--;
#ifdef DEBUG
		printf("\033[22;31m\n>>>>DEBUG: This element is identical to the median\n\033[0m");
#endif
		return;
	} else {
		struct set_node* n = s->max;
		while(n->next != NULL && n->next != s->med && ((n->next)->value)->F > new->F) {
			n = n->next;
		}
		if (n->next != NULL && are_equal((n->next)->value,new,s->id_size)) {
			((n->next)->ttl)--;
			return;
		}
		elem->next = n->next;
		elem->prev = n;
		if (n->next != NULL) (n->next)->prev = elem;
		n->next = elem;
#ifdef DEBUG
		printf("\033[22;31m\n>>>>DEBUG: This element is lower than the median\n\033[0m");
#endif
		if (is_even(s->size)) s->med = s->med->prev;
	}
	s->size++;
...
#ifdef DEBUG
		printf("\033[22;31m\n>>>>DEBUG: The element out is the median\n\033[0m");
#endif
		if (s->size == 2) {
			s->min = s->max;
			s->med = s->max;
			s->max->next = NULL;
#ifdef DEBUG
			printf("\033[22;31m\n>>>>DEBUG: The set is now of size 1\n\033[0m");
#endif
		} else if (is_even(s->size)) {
			s->med = s->med->prev;
			s->med->next = n->next;
			n->next->prev = s->med;
#ifdef DEBUG
			printf("\033[22;31m\n>>>>DEBUG: The set was of even size\n\033[0m");
#endif
		} else {
			s->med = s->med->next;
			s->med->prev = n->prev;
			n->prev->next = s->med;
#ifdef DEBUG
		printf("\033[22;31m\n>>>>DEBUG: The set was of odd size\n\033[0m");
#endif
		}
	} else if (n == s->min) {
#ifdef DEBUG
		printf("\033[22;31m\n>>>>DEBUG: The element out is the minimum\n\033[0m");
#endif
		s->min = s->min->prev;
		s->min->next = NULL;
		if (is_even(s->size)) s->med = s->med->prev;
...
#endif
		if ((((n->value)->F < ((s->med)->value)->F) || medseen) && is_even(s->size)) {
			s->med = s->med->prev;
#ifdef DEBUG
			printf("\033[22;31m\n>>>>DEBUG: The element out is after the median (even size)\n\033[0m");
#endif
		} else if ((n->value)->F > ((s->med)->value)->F && is_odd(s->size)) {
			s->med = s->med->next;
#ifdef DEBUG
			printf("\033[22;31m\n>>>>DEBUG: The element out is before the median (odd size)\n\033[0m");
#endif
		}
	}
	s->size--;
