...
%    (Timeout > 0
%    ->  catch(call_with_time_limit(Timeout,findall(Outcome, schedule(TasksOrd,Outcome,0,F), Outcomes)),_,schedule(TasksOrd,Outcome,0,F)) % SRM: We take care of the algorithm timeout
%    ;   findall(Outcome, schedule(TasksOrd,Outcome,0,F), Outcomes)),
	!,
	schedules(3,TasksOrd,[Outcome|Outcomes],0,F),
    %findall(Outcome, schedule(TasksOrd,Outcome,0,F), Outcomes),
